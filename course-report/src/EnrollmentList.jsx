...
function EnrollmentList() {
    ...
    // STore the currently selected status from drop down.
    const [statusFilter, setStatusFilter] = useState(''); // Track selected status filter
    ...
    const filteredEnrollments = enrollments.filter((enrollment) => {
        ...
        const matchesStatusFilter = statusFilter === '' || enrollment.completion_status === statusFilter;

        return matchesSearchTerm && matchesStatusFilter;  // Ensure both conditions match
    });
    ...
    // Get the current enrollments for the current page
    const currentEnrollments = sortedEnrollments.slice(indexOfFirstEnrollment, indexOfLastEnrollment);


    // Calculate the total number of pages
    // Calculate the total number of pages based on filtered enrollments
    const totalPages = Math.ceil(filteredEnrollments.length / enrollmentsPerPage);

    // Ensure currentPage is within valid bounds after filtering
    useEffect(() => {
        if (currentPage > totalPages) {
            setCurrentPage(1);
        }
    }, [filteredEnrollments, currentPage, totalPages]);
            ...
            <div className="container mx-auto p-6 shadow-lg bg-white min-h-[800px]">
                ...
                <div className='flex justify-between items-center mb-4'>
                    ...
                    <input
                        ...
                        className="w-9/12 p-2 border border-gray-300 rounded-lg"
                        value={searchTerm}
                    ...
                    <select
                        className="ml-3 w-3/12 p-2 border-2 border-customGreen rounded-lg"
                        value={statusFilter}
                        ...
                        <tbody>
                            {currentEnrollments.map((enrollment, index) => (
                                <tr key={index} className="text-gray-700 hover:bg-gray-100">
                                    ...
                                    <td className={`${enrollment.completion_status === 'completed'
                                            ? 'text-green-600 p-3'
